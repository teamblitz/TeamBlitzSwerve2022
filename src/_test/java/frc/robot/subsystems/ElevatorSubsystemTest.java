...
public class ElevatorSubsystemTest {
    ...
    public void setup() {
        ...
        // This must be spy not mock. Mock makes void methods not do anything while this does not disrupt their normal behavior
    ...
    public void checkThatUpMakesElevatorGoUp() {
        ...
        assertTrue(master.get() >= Math.min(0, ElevatorConstants.kUpSpeed) && master.get() <= Math.max(0, ElevatorConstants.kUpSpeed) && master.get() != 0);
        // assertTrue(slave.get() >= Math.min(0, ElevatorConstants.kUpSpeed) && slave.get() <= Math.max(0, ElevatorConstants.kUpSpeed) && slave.get() != 0);
    }
    @Test
    public void checkThatDownMakesElevatorGoDown() {
        ...
        assertTrue("Checks Speed is within acceptable range", master.get() >= Math.min(0, ElevatorConstants.kDownSpeed) && master.get() <= Math.max(0, ElevatorConstants.kDownSpeed) && master.get() != 0);
        //assertTrue("Checks Speed is within acceptable range", slave.get() >= Math.min(0, ElevatorConstants.kDownSpeed) && slave.get() <= Math.max(0, ElevatorConstants.kDownSpeed) && slave.get() != 0);
    }
    @Test
